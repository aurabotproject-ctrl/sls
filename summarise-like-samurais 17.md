...
Flipping the toggle only swaps which `STORIES` array is active (`STORIES_CHRISTIAN` vs `STORIES_JAPANESE`) and resets to that curriculum's Story 1 — it does not touch ranks, badges, backdrops, fonts, or any other art. Only the **Story Scene** hero image (Section 11.D) differs per curriculum, since it depicts a moment from that specific story. As of this build, **Story 1 (Momotaro / Peach Boy)** is the only Japanese Legends story with the full 5-lesson interactive cycle built — see Section 15.1.
...
### Rank 2 — Novice Swordsman (Stories 6–10 / Lessons 26–50): Gratitude & Kindness
...
9. **Urashima Taro** — *"A silhouetted young fisherman riding on the back of a large sea turtle beneath the waves, glowing sunset light filtering down through the water above them, a distant reef and cluster of sea plant silhouettes below."* (tell as a pure fantasy undersea-kingdom story about the passage of time and keeping promises — no deity/ruler-worship framing)
...
- [ ] Write all 40 stories in 3 scroll levels each, each scroll broken into exactly 3 matching paragraphs (120 total text pieces) — 5 stories (Story 1–5) are done
...
- [x] Build the Story Scene Template (Section 11.D) for all 5 built stories (1–5) — generated, compressed to webp, and wired in as `STORY_SCENES[idx]`, shown as a hero banner in the lesson panel and swapped by `setActiveStory()`
...
- [x] "Print all readings" button that scales automatically as new stories are added to `STORY_DATA` — now prints all 5 built stories back to back (15 pages)
- [x] Stories 2–5 pushed into `STORY_DATA`, so both print buttons cover all 5 built stories without extra wiring — as Stories 6–40 are built, push each new story in the same way
...
- [ ] Extend the same pattern to Stories 6–40
...
| **🖨️ Print all readings** | Prints every built story's readings back to back (one story = three pages, one per scroll level). Now prints all 5 built stories (15 pages total) — the code loops over the `STORY_DATA` array, which is derived from `STORIES`, so as Stories 6–40 are added, each new story's reading is picked up by both this button and the per-lesson print button automatically. No per-story wiring is needed. |
...
**Story Scenes (Section 11.D) — now generated and wired in for all five built stories.** A silhouette hero illustration was generated for **Story 1 (Creation)**, **Story 2 (Noah's Ark)**, **Story 3 (Abraham's Call & Journey)**, **Story 4 (Joseph's Coat & Dreams)**, and **Story 5 (Joseph Forgives His Brothers)**, each resized to 900px wide and compressed to webp (~53–63KB each), embedded base64 as `STORY_SCENES[idx]`, and shown as a full-width hero banner in the "Story one: the full five-lesson cycle" panel, right above the lesson tabs, swapping automatically with `setActiveStory()` as the Story tab changes. The Lesson 1 Presentation Mode slide also picks up the same image for its opening slide. Once Stories 6–40 are built, each will need its own Story Scene image generated from the reusable template prompt (Section 11.D) before it gets a hero banner — the collapse-cleanly behaviour (no broken image, no empty gap) stays in place for any future story that doesn't have one yet.
...
- Five full interactive lesson demos, one per built story, each with the full 5-lesson cycle (Rank 1, Phase 1 heavy scaffolding): **Story 1 — Creation**, **Story 2 — Noah's Ark**, **Story 3 — Abraham's Call & Journey**, **Story 4 — Joseph's Coat & Dreams**, **Story 5 — Joseph Forgives His Brothers**. A Story tab bar switches between them; each story's Lesson 1 has the student read the whole three-paragraph story once and answer comprehension questions, with no summarising yet (that starts in Lessons 2–4). Every story is written in three matching paragraphs across all three reading levels, at the widened word-count bands in Section 5.
- **Print & teacher tools, for every built story:** a "Print this reading" button (prints the current story's three differentiated scrolls, one per page), a "Print all readings" button (prints all 5 built stories back to back via the `STORY_DATA` array), and a "Teacher lesson plan" button opening a full NZC-linked lesson plan (Years 5–6, Phase 2 English) for the current story and lesson, with its own print option. See "Printing & Teacher Lesson Plans" above for details.
...
- **The full five-lesson cycle for Stories 1–5, and the `STORIES`/`setActiveStory()` multi-story architecture** — see Section 15 for full details.
- Not yet built: Stories 6–40 (Stories 1–5 are built on the new reusable, data-driven `STORIES`/`setActiveStory()` template), the belt ceremony animation, XP/streak systems, Master's Wall gallery, and the final certificate generator (see the Quick Build Checklist in Section 12)

---

## 15. The Full Five-Lesson Cycle, and the Multi-Story Architecture (Stories 1–5)

This is the newest layer: five stories — Creation, Noah's Ark, Abraham's Call & Journey, Joseph's Coat & Dreams, and Joseph Forgives His Brothers — each have their **full five-lesson cycle** built, wired together, and playable end to end, and the app was refactored so any of them can be switched into the same five lesson panels on demand.

### The Story tab bar

A new row of tabs sits above the lesson tabs, one per built story ("Story 1 · Creation" through "Story 5 · Joseph Forgives His Brothers"). Clicking one calls `setActiveStory(idx)`, which re-points every module-level data variable at that story and re-renders the whole lesson demo in place — the lesson tab bar underneath it always resets back to Lesson 1 on a story switch, so the student always lands on "Read & Understand" for whichever story they just picked.
...
4. **Master's Seal** — the closing "Story N complete!" moment, with a preview line naming the next built story by title (or "coming soon" once Story 5's cycle finishes, since Story 6 isn't built yet).
...
- `buildStoryPrintPages(story)` (unchanged) still powers both "Print this reading" (now `STORY_DATA[currentStoryIdx]`, so it prints whichever story tab is open) and "Print all readings" (still `STORY_DATA.map(...)`, so it now prints all 5 built stories back to back — 15 pages, one per scroll level per story).
...
- **`STORY_DATA`** (used for printing) is now derived with `STORIES.map(...)` instead of being hand-written, so it always mirrors the 5 built stories with no duplication.

Tested end to end: cycling through all 5 story tabs × all 5 lesson tabs (25 combinations) with zero console/JS errors; typing in Story 1's retell box, switching to Story 2 and confirming its retell box is empty, then switching back to Story 1 and confirming the earlier text persisted (and that only `ssl_story1_...`/`ssl_story2_...` keys existed in `localStorage`, never a shared one); and printing both the reading and worksheet for Story 1 and Story 3, confirming the same page-count limits from Section 13 still hold (each scroll level still fits one printed page, each worksheet still fits within 2 pages) now that the print HTML is generated from per-story data instead of one hard-coded story.

### Build notes for extending this to Stories 6–40

The architecture above should carry Stories 6–40 with no further refactor — the only work left is content: write each new story to the same `story_content.py`-style shape (`scrolls` in 3 levels × 3 matching paragraphs, `searchWords`/`orderEvents`/`factStatements`, and `para["2"/"3"/"4"]` with `label`/`search`/`order`/`remove`/`starter`), then append it to the `STORIES` array and add one more button to the story tab bar. Nothing in `setActiveStory()`, the lesson-plan/worksheet/presentation builders, or `STORY_DATA` needs to change per new story — they all already read from `STORIES` generically.
